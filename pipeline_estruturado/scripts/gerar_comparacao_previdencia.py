...
def numeric_or_none(
    ...
    """
    Converte um valor escalar em float.

    Valores ausentes ou não numéricos são
...
def normalize_status(
    ...
    """
    Normaliza textos utilizados nos campos de status.
    """
    if value is None:
        return ""
...
def build_multisource_consistency(
    ...
    O alerta é gerado quando a divergência é
    estritamente maior que o limite definido.

    Períodos parciais, valores ausentes e referência
    igual a zero não geram alerta.
    """
    ...
    if missing_columns:
        raise ValueError(
            "A comparação não possui as colunas "
            "necessárias para a análise de consistência: "
            f"{missing_columns}."
        )
    ...
    for _, row in comparison.iterrows():
        ...
        if reference_value is None or compared_value is None:
            ...
            observation = (
                "O exercício não foi avaliado porque "
                "uma das fontes possui período parcial "
                "ou data de corte diferente."
            )
            ...
            observation = (
                "A divergência percentual não foi "
                "calculada porque o valor de referência "
                "é igual a zero."
            )
            ...
            divergence_percent = absolute_difference / abs(reference_value) * 100
            ...
            if alert:
                ...
                observation = "A divergência permaneceu dentro do limite definido."
...
def main() -> None:
    ...
    if consistency.empty:
        raise RuntimeError("A análise de consistência entre as fontes não foi gerada.")
    ...
    alerts = consistency.loc[consistency["alerta"].eq(True)]
    ...
    if alerts.empty:
        print("\nNenhuma divergência superior a 10% foi identificada.")
